...
1. **内存缓存**：模块加载时（或首次访问时）从 store 读取并校验一次，得到 `Conversation[]` 缓存于内存；后续 `list/get/create/rename/delete/saveMessages/setPinned/setArchived` 都基于内存缓存读，写时更新缓存并落盘。消除每次操作的全量重复校验。
2. **去抖合并落盘**：把"更新内存缓存"与"写磁盘"分离；磁盘写入做短延时去抖（如 150–250ms）合并连续写（tool-event 紧接 finish 等）。关键：进程退出（`app.on('before-quit')` / `will-quit`）前 flush，保证不丢数据；测试中可注入/直接 flush。
3. **损坏先备份**：读取时若 store 数据非法/无法校验，将原始文件复制为 `*.corrupt-<timestamp>.json` 再回退到空，避免静默清空用户数据。
4. 保持对外导出的函数签名不变（`listConversations` / `getConversation` / `saveMessages` 等），仅改内部实现，IPC 层与渲染层无需改动。

**验证**：扩充 `src/main/__tests__/conversationStore.test.ts`：
- 多次 `saveMessages` 后磁盘写入被合并 / 数据最终一致。
- 缓存与磁盘一致：写后重新 `listConversations` 反映最新。
- 损坏输入触发备份文件生成且不清空到生产路径之外（用临时目录）。
- 退出前 flush 不丢数据。
- `npm test` + `npm run typecheck` 通过。

**接受标准**：单次操作不再 O(全历史) 重复校验；连续写合并；损坏有备份；退出不丢数据；现有测试与新测试通过；对外行为不变。

**风险与取舍**：去抖引入"内存已改、磁盘未落"的窗口。通过退出前 flush + 写失败回退在内存保留来缓解。若实现复杂度偏高，可先只做"内存缓存 + 损坏备份"，去抖作为可选项；最终以测试通过和不丢数据为准。
