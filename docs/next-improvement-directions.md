...
1. 下一阶段是否确认选择 0.3：会话资产化 + 使用可靠性？
2. 0.3 是否需要包含“导入”，还是只做导出？
3. 搜索只做当前本地 JSON 线性搜索，还是直接设计未来可迁移的搜索接口？
4. Usage 汇总是否要估算价格，还是只显示 tokens？
5. 发布工程增强是否穿插做“检查更新”，还是推迟到 0.4 之后？

## 当前建议

确认 0.3 作为下一阶段后，再写正式设计文档。正式设计应覆盖：
