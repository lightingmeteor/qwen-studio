...
2. 下载 Windows 安装包 `Qwen.Studio.Desktop.Setup.0.4.1.exe`。
3. 双击运行 `.exe` 安装包。
...
- API 模式：支持在 Chat Completions（默认、稳定路径）和 Responses API 之间切换；Responses 模式会使用 `previous_response_id` 传递上下文，id 失效或过期时自动回退为全量历史重发，无需手动干预。
...
- 会话分叉：可从任意已完成消息"从这里分叉"出新会话，原会话保持不变；编辑消息时可选择「分叉后发送」，在新时间线上尝试不同问法。
- 分叉来源：分叉会话顶部显示"分叉自 xxx 第 N 条"横幅，点击可跳回原会话并定位到分叉点；原会话删除后横幅自动降级为纯文本。
...
- 删除会话：鼠标移到会话上，点击删除图标。
- 复制消息：鼠标移到消息上，点击 `复制`。
- 编辑消息：鼠标移到自己的消息上，点击 `编辑`，修改后可选 `重新发送`（截断后续内容并原地重发）或 `分叉后发送`（在分叉出的新会话中发送，原会话不变）。
- 分叉会话：鼠标移到任意已完成消息上，点击 `从这里分叉`，会复制到该消息为止的内容并切换到新会话。
- 跳回分叉来源：在分叉会话顶部点击 `分叉自 xxx 第 N 条` 横幅，可回到原会话并定位到分叉点。
...
当前 0.4.x 只接入 `web_search`。暂不包含 `web_extractor`、`code_interpreter`、文件 / Qwen-Long、多模态输入、知识库搜索或自定义函数。
