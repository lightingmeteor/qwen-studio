...
function CodeBlock({ children }: { children?: ReactNode }): JSX.Element {
  ...
  return (
    <div className="relative group my-2 min-w-0">
      ...
      <pre className="max-w-full overflow-x-auto rounded-lg bg-[#0b0d12] p-3 text-sm">{children}</pre>
    </div>
  );
}
...
function ExternalLink({ href, children }: { href?: string; children?: ReactNode }): JSX.Element {
  ...
  return (
    <a
      ...
      onClick={(e) => {
        e.preventDefault();
        if (safeUrl) void window.qwen.openExternal(safeUrl).catch(console.error);
      }}
